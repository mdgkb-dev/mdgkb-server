alter table residency_courses
    add main_teacher_id uuid;

alter table residency_courses
    add constraint residency_courses_employees_id_fk
...
SELECT s.name, s.code, s.slug,
       sy.year as start_year,
       ey.year as end_year,
       rc.*
...
group by rc.id, s.name, s.code, s.slug, sy.year, s.code, s.slug, s.name, ey.year, sy.id, ey.id;
